...
class MCPResultProcessor:
    ...
    def __init__(self):
        ...
        self.llm = ChatOpenAI(
            ...
            api_key="sk-490fc6a12924409489eea7ee17b8f714",
        )
        ...
        self.summary_prompt = ChatPromptTemplate.from_messages(
            [
                (
                    "system",
                    """你是一个专业的信息摘要专家。请对提供的工具执行结果进行智能摘要。
...
输出格式：结构化的摘要内容，便于用户快速理解。""",
        ...
        self.format_prompt = ChatPromptTemplate.from_messages(
            [
                (
                    "system",
                    """你是一个专业的内容格式化专家。请将工具执行结果格式化为用户友好的展示格式。
...
输出格式：格式化后的结果内容。""",
    ...
    async def _summarize_result(
        self, tool_name: str, result: str, options: Dict[str, Any]
    ) -> str:
        """生成结果摘要"""
        ...
        print("_summarize_result")
        summary = await chain.ainvoke({"tool_name": tool_name, "result": result})
    ...
    def _filter_result(self, result: str, options: Dict[str, Any]) -> str:
        ...
        for filter_name in filters:
            if filter_name == "remove_empty_lines":
                filtered_result = "\n".join(
                    [line for line in filtered_result.split("\n") if line.strip()]
                )
...
async def process_mcp_result(
    tool_name: str, result: str, mode: str = "formatted", **options
) -> str:
    """
    便捷的MCP结果处理函数
    ...
    return await result_processor.process_result(
        tool_name, result, processing_mode, options
    )
